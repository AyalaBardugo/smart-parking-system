...
public class ParkingLot : IDisposable
{
    ...
    public async Task<ParkingResult> TryEnterAsync(Vehicle vehicle)
    {
        ...
        // Wait until spot becomes available (controlled by semaphore)
        await _capacity.WaitAsync();

        var success = false;
        try
        {
            // TryDequeue is atomic - only one thread gets each spot
            if (!_availableSpots.TryDequeue(out var freeSpot))
                return ParkingResult.NoSpotsAvailable();
            
            //  At this point we have a reserved spot from available queue
            // Should never fail, but handle edge case
    ...
    public Task<UnparkingResult> ExitAsync(string licensePlate)
    {
        ...
        // TryRemove ensures only one thread processes each vehicle exit
        ...
        try
        {
            // Prevent double-exit of same session
            if (!session.IsActive)
            {
                _activeSessions.TryAdd(licensePlate, session);
